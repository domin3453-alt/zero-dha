...
export async function POST(request: Request) {
  try {
    ...
    // Persist fields as entered — do not copy Buy↔Sell or LTP↔Sell.
    const normalizedOrders = (Array.isArray(config.orders) ? config.orders : []).map(
      (o) => {
        const buy = Number(o.buyPrice ?? 0);
    ...
    const normalizedConfig: OrdersConfig = {
      ...config,
